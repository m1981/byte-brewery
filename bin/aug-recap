...
def stream_conversation_recap(
    ...
    if validate_output:
        # Get the full output
        if isinstance(output_buffer, StringIO) and output_buffer:  # Check output_buffer instead
...
USAGE EXAMPLES:
  ...
  1. Generate a Clean Report (Recommended)
     $ ./aug-recap output/chat_state.json -v --validate -o recap.md
     
     > Generates a full report with verbose logging (-v).
     > The --validate flag ensures all code blocks (```) are properly closed, 
       preventing broken Markdown rendering in large files.

  2. Daily Standup Summary
     $ ./aug-recap output/chat_state.json --since 2025-01-01 -o daily_update.md
     
     > Filters the history to show only conversations created or modified 
       on or after the specified date.
     > Perfect for generating "What I did this week" reports.

  3. Debugging Truncation Logic
     $ ./aug-recap output/chat_state.json --debug-truncation -o debug.md
     
     > If your report feels like it's missing content, use this flag.
     > It prints detailed logic to stderr explaining why specific text blocks 
       were shortened or summarized.

  4. Full Fidelity (Show All Edits)
     $ ./aug-recap output/chat_state.json --show-edits > full_log.md
     
     > By default, file edits are summarized to save space.
     > Use --show-edits to include the actual code changes in the output.
-------------------------------------------------------------------------
        """
    )
    ...
    if args.output:
        ...
        stream_conversation_recap(
            json_data, sys.stdout, args.since, args.show_edits, args.verbose,  # Use sys.stdout instead of f
